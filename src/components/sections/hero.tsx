"use client"

import Link from "next/link"
import Image from "next/image"
import { Button } from "@/components/ui/button"
import { Phone } from "lucide-react"
import { CONTACT_PHONE } from "@/lib/constants"

export function Hero() {
  return (
    <section className="relative min-h-screen w-full flex items-center justify-center py-20 sm:py-24">
      {/* Boxed Image Container */}
      <div className="relative z-10 w-full max-w-[95%] xl:max-w-[98%] mx-auto px-3 sm:px-4 lg:px-6">
        <div className="relative rounded-2xl sm:rounded-3xl border border-white/30 bg-background/10 backdrop-blur-xl shadow-2xl overflow-hidden w-full min-h-[700px] sm:min-h-[800px] md:min-h-[900px] ring-1 ring-white/20">
          {/* Background Image */}
          <Image
            src="/header.png"
            alt="NWS Group AG"
            fill
            className="object-cover object-center"
            sizes="100vw"
            priority
          />
          <div className="absolute inset-0 bg-gradient-to-br from-black/70 via-black/50 to-black/70" />
          
          {/* Content */}
          <div className="relative z-10 flex h-full flex-col items-center justify-center text-center text-white p-6 sm:p-8 md:p-12 lg:p-16">
            <h1 className="mb-3 sm:mb-4 text-2xl sm:text-3xl md:text-5xl lg:text-6xl xl:text-7xl font-display leading-tight tracking-tight">
              Ihr Partner für Autoankauf,
              <br />
              <span className="bg-gradient-to-r from-white via-white to-white/80 bg-clip-text text-transparent">-verkauf & Karosseriearbeiten</span>
            </h1>

            <p className="mb-6 sm:mb-8 text-base sm:text-lg md:text-xl lg:text-2xl">
              An zwei Standorten in Pratteln und Dornach für Sie da
            </p>

            <div className="mb-6 sm:mb-8 flex flex-col items-center gap-3 sm:gap-4 sm:flex-row">
              <Button size="lg" className="w-full sm:w-auto h-11 sm:h-12 text-sm sm:text-base" asChild>
                <Link href="/kontakt?type=verkauf">
                  Auto verkaufen
                </Link>
              </Button>
              <Button size="lg" variant="outline" className="glass border-white/30 text-white hover:bg-white/20 hover:border-white/50 w-full sm:w-auto h-11 sm:h-12 text-sm sm:text-base transition-all duration-300" asChild>
                <Link href="/fahrzeuge">Fahrzeuge ansehen</Link>
              </Button>
            </div>

            <a
              href={`tel:${CONTACT_PHONE.replace(/\s/g, "")}`}
              className="flex items-center justify-center gap-2 text-base sm:text-lg md:text-xl font-semibold hover:text-primary transition-colors"
            >
              <Phone className="h-5 w-5 sm:h-6 sm:w-6" />
              {CONTACT_PHONE}
            </a>
          </div>
        </div>
      </div>
    </section>
  )
}

